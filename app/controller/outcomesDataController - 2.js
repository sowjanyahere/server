...
let outcomesDataController = {
  ...
  goal_types_firstName: async function (req, res) {
  ...
  goal_type_yearly: async function (req, res){
    try {
        const yearGoalTypes = await outcomeDataModel.aggregate([
            {
              $unwind: "$goal"
            },
            {
